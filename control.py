...
import sys, getopt, urllib, http.client, re
...
def usage():
    ...
    print(
        "  -p, --priority=PRIORITY\tPriority of the job (default: "
        + str(priority)
        + ")"
    )
    print(
        "  -r, --retry=RETRY\tNumber of retry this jobs can do (default: "
        + str(retry)
        + ")"
    )
    print(
        '  -a, --affinity=AFFINITY\tAffinity words to workers, separated by a comma (default: ""'
    )
...
if action == "add":
    ...
    conn = http.client.HTTPConnection(re.sub("^http://", "", serverUrl))
    ...
    params = urllib.parse.urlencode({"id": parent})
    conn = http.client.HTTPConnection(re.sub("^http://", "", serverUrl))
    ...
    if id < 0:
        ...
        params = urllib.parse.urlencode({"id": id})
        conn = http.client.HTTPConnection(re.sub("^http://", "", serverUrl))
