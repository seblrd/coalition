...
import sqlite3, pymysql, unittest, time, re, sys
from db import DB
...
class DBSQL(DB):
        ...
        with self.Conn:
            ...
            for i in range(1, 64):
                if not i in affinities:
                    self._execute(
                        cur, "INSERT INTO Affinities (id, name) VALUES (%d,'')" % i
                    )
    ...
    def _getLdapPermission(self, action):
        ...
        if (
            not hasattr(self, "ldap_user") or not self.ldap_user
        ):  # LDAP is not set up in configuration or ldapunsafeapi is set to True
            return ""
        if action == "addjob":
            if self.permissions["ldaptemplateaddjobglobal"]:
                ...
                raise LdapError(
                    "Action '{action}' is not permitted for user '{user}'".format(
                        action=action, user=self.ldap_user
                    ),
                    False,
                )
                return False
            ...
            raise LdapError(
                "Action '{action}' is not defined for user '{user}'".format(
                    action=action, user=self.ldap_user
                )
            )
            return False
    ...
    def setAffinities(self, affinities):
        ...
        for id, affinity in affinities.iteritems():
            self._execute(
                cur,
                "UPDATE Affinities SET name = '%s' WHERE id = %d" % (affinity, int(id)),
            )
    ...
    def newJob(
        ...
        self._execute(
            ...
            "FROM Jobs "
            "WHERE id = {parent} {ldap_perm}".format(
                parent=parent, ldap_perm=ldap_perm
            ),
        )
        data = cur.fetchone()
        paused = 0
        ...
        if data[4] != "":
            print(
                "Error: can't add job, parent {parent} is not a group".format(
                    parent=parent
                )
            )
            return None
        ...
        self._execute(
            ...
            "{progress_pattern}, {paused}, {state}, {worker}, {h_depth},"
            "{h_affinity}, {h_priority}, {h_paused})".format(
                parent=parent,
                title=repr(title),
                ...
                child_affinities=child_affinities,
                user=repr(user),
                url=repr(url),
    ...
    def getCountJobsWhere(self, where_clause=""):
        ...
        self._execute(
            cur,
            "SELECT COUNT(*) FROM Jobs WHERE {}".format(
                where_clause[0].decode("utf-8")
            ),
        )
    ...
    def getChildrenDependencyIds(self, id):
        ...
        self._execute(
            cur,
            "SELECT job.id AS id, dep.dependency AS dependency FROM Dependencies AS dep "
    ...
    def getWorker(self, hostname):
        ...
        self._execute(
            cur,
            "SELECT affinity FROM WorkerAffinities WHERE worker_name = '%s'"
            % (hostname),
        )
        affinities = []
    ...
    def editJobs(self, jobs):
        ...
        for id, attr in jobs.iteritems():
            if (
                attr.has_key("user")
                and attr["user"].lower() != self.ldap_user.lower()
                and ldap_perm != ""
            ):
                # User has no global permission, so he can change his own jobs only
                raise LdapError(
                    "User '{user}' is not allowed to change job id='{id}' user attribute to '{user_attr}'".format(
            ...
            if (
                attr.get("parent") is not None
                or attr.get("affinity") is not None
                or attr.get("priority") is not None
                or attr.get("paused") is not None
            ):
    ...
    def setJobProgress(self, jobId, progress):
        cur = self.Conn.cursor()
        self._execute(
            cur, "UPDATE Jobs SET progress = %f WHERE id = %d" % (progress, jobId)
        )
    ...
    def _getMigrationVersion(self):
        ...
        return int(
            max([re.sub(r"_.*$", "", f) for f in next(iter(os.walk("migrations")))[2]])
        )
    ...
    def migrateDatabase(self, init=False):
        ...
        cur = self.Conn.cursor()
        print(
            "The database version is {current} and the migration target is {target}. Migrating.".format(
                current=current, target=target
            )
        )
        while current < target:
            current += 1
            migration_module_name = "{current:04d}_db_{db_type}".format(
                current=current, db_type=db_type
            )
            migration_module = import_module(
                "migrations.{}".format(migration_module_name)
            )
            with self.Conn:
    ...
    def moveJob(self, jobId, parent):
        ...
        self._execute(
            cur, "UPDATE Jobs SET parent = %d WHERE id = %d" % (parent, jobId)
        )
    ...
    def setJobAffinity(self, id, affinity):
        ...
        self._execute(
            cur,
            "UPDATE Jobs SET affinity = '%s', affinity_bits = %d WHERE id = %d"
    ...
    def setJobPriority(self, id, priority):
        ...
        priority = max(0, min(255, int(priority)))
        self._execute(
            cur, "UPDATE Jobs SET priority = %d WHERE id = %d" % (priority, id)
        )
    ...
    def setJobDependencies(self, id, dependencies):
        ...
        for dep in dependencies:
            self._execute(
                cur,
                "INSERT INTO Dependencies (job_id,dependency) "
    ...
    def resetJob(self, id, updateChildren=True):
        ...
        if ldap_perm != "":  # Not a global permission
            ...
            if data and data[0].lower() != self.ldap_user.lower():
                raise LdapError(
                    "User '{user}' is not allowed to reset job id='{id}'".format(
                        user=user, id=id
                    )
                )
                return None
    ...
    def resetErrorJob(self, id, updateChildren=True):
        ...
        if ldap_perm != "":  # Not a global permission
            ...
            if data and data[0].lower() != self.ldap_user.lower():
                raise LdapError(
                    "User '{user}' is not allowed to reset error job id='{id}'".format(
                        user=user, id=id
                    )
                )
                return None
    ...
    def startJob(self, id):
        ...
        if ldap_perm != "":  # Not a global permission
            ...
            if data and data[0].lower() != self.ldap_user.lower():
                raise LdapError(
                    "User '{user}' is not allowed to start job id='{id}'".format(
                        user=user, id=id
                    )
                )
                return None
    ...
    def pauseJob(self, id):
        ...
        if ldap_perm != "":  # Not a global permission
            ...
            if data and data[0].lower() != self.ldap_user.lower():
                raise LdapError(
                    "User '{user}' is not allowed to pause job id='{id}'".format(
                        user=user, id=id
                    )
                )
                return None
    ...
    def deleteJob(self, id, deletedJobs=[], updateCounters=True):
        ...
        if ldap_perm != "":  # Not a global permission
            ...
            if data and data[0].lower() != self.ldap_user.lower():
                raise LdapError(
                    "User '{user}' is not allowed to delete job id='{id}'".format(
                        user=user, id=id
                    )
                )
                return None
    ...
    def newWorker(self, name):
        ...
        self._execute(
            cur,
            "INSERT INTO Workers (name,ip,affinity, state,finished,"
    ...
    def setWorkerAffinity(self, name, affinity):
        ...
        self._execute(
            cur, "DELETE FROM WorkerAffinities WHERE worker_name = '%s'" % (name)
        )
        ...
        if len(affinity) > 0:
            ...
            for index, aff in enumerate(affinities):
                ...
                query = (
                    "INSERT INTO WorkerAffinities ( worker_name, affinity, ordering ) VALUES( '%s', %d, %d )"
                    % (name, self.getAffinityMask(aff), index + 1)
                )
    ...
    def heartbeat(self, hostname, jobId, cpu, free_memory, total_memory, ip):
        ...
        _query = (
            "SELECT w.active, w.state, j.state FROM Workers as w "
        ...
        self._execute(
            cur, "SELECT active, state FROM Workers WHERE name = '%s'" % hostname
        )
        worker = cur.fetchone()
        if worker is None:
        ...
        if worker[0] == True:
            self._execute(
                cur,
                "SELECT state, h_paused FROM Jobs WHERE id = %d AND worker = '%s'"
                % (jobId, hostname),
            )
            job = cur.fetchone()
    ...
    def pickJob(self, hostname, cpu, free_memory, total_memory, ip):
        ...
        if job is None:
            ...
            if job is None:
                ...
                self._execute(
                    cur,
                    "UPDATE Workers SET state = 'WAITING' WHERE name = '{}'".format(
                        hostname
                    ),
                )
                return -1, "", "", "", None
    ...
    def endJob(self, hostname, jobId, errorCode, ip):
        ...
        self._execute(
            cur,
            "SELECT state, start_time FROM Jobs WHERE id = %d AND worker = '%s' AND state = 'WORKING'"
            % (jobId, hostname),
        )
        job = cur.fetchone()
        if job is not None:
            ...
            self._execute(
                cur,
                "UPDATE Events SET state = %s, duration = %d WHERE id = %d"
    ...
    def _isJobPending(self, id):
        ...
        self._execute(
            cur,
            "SELECT COUNT(job.id) FROM Jobs AS job "
    ...
    def _setJobState(self, id, state, updateCounters):
        ...
        cur = self.Conn.cursor()
        self._execute(
            cur, "SELECT state, parent, user, title, id FROM Jobs WHERE id = %d" % id
        )
        job = cur.fetchone()
        if job is not None:
    ...
    def _updateJobCounters(self, id, updateParent=True):
        if id != 0:
            ...
            self._execute(
                cur,
                "SELECT state, total_working, total_errors, total_finished, total, start_time, duration FROM Jobs WHERE parent = %d"
                % id,
            )
            for job in cur:
                state = job[0]
            ...
            if total > 0:
                self._execute(
                    cur,
                    "SELECT state, parent, user, title, id, progress FROM Jobs WHERE id = %d"
                    % id,
                )
                ...
                if newState != oldState[0]:
                    ...
                    self._execute(
                        cur,
                        "UPDATE Jobs SET state = '%s' WHERE id = %d" % (newState, id),
                    )
                ...
                if progress != oldState[5]:
                    self._execute(
                        cur,
                        "UPDATE Jobs SET progress = %f WHERE id = %d" % (progress, id),
                    )
    ...
    def _setWorkerState(self, hostname, state):
        cur = self.Conn.cursor()
        self._execute(
            cur, "SELECT state FROM Workers AS worker WHERE name = '%s'" % hostname
        )
        worker = cur.fetchone()
        if worker is not None and worker[0] != state:
            if state == "ERROR":
                self._execute(
                    cur,
                    "UPDATE Workers SET state = 'WAITING', error = error+1 WHERE name = '%s'"
                    % hostname,
                )
            elif state == "TIMEOUT":
                self._execute(
                    cur,
                    "UPDATE Workers SET state = 'TIMEOUT', error = error+1 WHERE name = '%s'"
                    % hostname,
                )
            elif state == "FINISHED":
                self._execute(
                    cur,
                    "UPDATE Workers SET state = 'WAITING', finished = finished+1 WHERE name = '%s'"
                    % hostname,
                )
            else:
                self._execute(
                    cur,
                    "UPDATE Workers SET state = '%s' WHERE name = '%s'"
                    % (state, hostname),
                )
    ...
    def _updateChildren(self, id, parenth=None):
        ...
        self._execute(
            cur,
            "SELECT parent, affinity_bits, priority, paused, state FROM Jobs WHERE id = %d"
            % id,
        )
        job = cur.fetchone()
        if job:
            if not parenth:
                self._execute(
                    cur,
                    "SELECT h_depth, h_affinity, h_priority, h_paused FROM Jobs WHERE id = %d"
                    % job[0],
                )
            ...
            self._execute(
                cur,
                "UPDATE Jobs SET h_depth = %d, h_affinity = %d, h_priority = %d, h_paused = %d "
    ...
    def _update(self):
        ...
        if current_time - self.LastUpdate >= 10:
            ...
            if self.Verbose:
                print(
                    "[STAT] %d heartbeats, %d pickjobs, load %f"
                    % (self.HeartBeats, self.PickJobs, load)
                )
            self.HeartBeats = 0
            self.PickJobs = 0
            ...
            self._execute(
                ...
                "(timeout != 0 AND %d-start_time > timeout)" % current_time,
            )
            for job in cur:
            ...
            for worker in self.Workers:
                ...
                if current_time - info["ping_time"] > timeout and not info["timeout"]:
                    ...
                    self._execute(
                        cur,
                        "SELECT last_job FROM Workers WHERE name = '%s' AND state = 'WORKING'"
                        % worker,
                    )
                    data = cur.fetchone()
                    if data is not None:
            ...
            if self.cloudconfig:
                ...
                if len(waitingjobs):
                    self._manageWorkerInstanceStart(current_time, waitingjobs)
                else:
    ...
    def _manageWorkerInstanceTerminate(self, current_time):
		...
		"""
        uselessworkers = self.listWorkersByStates("STARTING", "WAITING", "TIMEOUT")
        if len(uselessworkers):
            for worker in uselessworkers:
                ...
                if lastworkerstarttime and (
                    current_time - lastworkerstarttime
                    > int(
                        self.cloudconfig.get(
                            "coalition", "workerinstanceminimumlifetime"
                        )
                    )
                ):
