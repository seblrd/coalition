...
def notifyError(job):
    if job["user"]:
        sendEmail(
            job["user"], "ERRORS in job " + job["title"] + " (" + str(job["id"]) + ")."
        )
...
def notifyFinished(job):
    if job["user"]:
        sendEmail(
            job["user"],
            "The job " + job["title"] + " (" + str(job["id"]) + ") is FINISHED.",
        )
...
def _interactiveConfirmation(confirmation_sentence="Yes I know what I'm doing."):
    ...
    text = (
        "Please write this sentence then press enter to confirm:\n"
...
def authenticate(request, ldap_permissions):
    ...
    def _getLdapPermissions(connection, username):
        ...
        def _ldapSearch(connection, query):
            if connection.search_ext_s(ldap_base, ldap3.SUBTREE, query, ["dn"]):
                return True
            return False
    ...
    if LDAPServer:
        ...
        if (
            config.has_option("server", "ldapunsafeapi")
            and config.getboolean("server", "ldapunsafeapi")
            and not isWebFrontend(request)
        ):
        ...
        if username or password:
            ...
            try:
                if conn.bind(ldapUsername):
                    vprint(
                        "[LDAP] Authentication accepted for user {}".format(username)
                    )
...
def grantAddJob(user, cmd):
    ...
    def checkWhiteList(wl):
        for pattern in wl:
            ...
            vprint(
                "[LDAP] Not authorized. User {} is not allowed to add the command {}".format(
                    user, cmd
                )
            )
        return False
...
class Master(xmlrpc.XMLRPC):
    ...
    def __init__(self):
        self.user = (
            ""  # Default value, overwritten later in case of LDAP authentication
        )
            ...
            if authenticated:
                ...
                if request.path == "/xmlrpc/addjob":
                    ...
                    try:
                        ...
                        def getChildArg(name, default):
                            if request.method == "GET":
                                # GET params
                                value = request.args.get(name, [default])[0]
                                value = type(default)(
                                    default if value == None else value
                                )
                                assert value != None
                                return value
                            else:
                            ...
                            if request.method == "PUT":
                                if request.path == "/api/jobs":
                                    if grantAddJob(
                                        ...
                                        job = db.newJob(
                                            (getChildArg("parent", 0)),
                                            (getChildArg("title", "")),
                                            (getChildArg("command", "")),
                                            (getChildArg("dir", "")),
                                            (getChildArg("environment", "")),
                                            (getChildArg("state", "WAITING")),
                                            (getChildArg("paused", 0)),
    ...
    def json_heartbeat(self, hostname, jobId, log, load, free_memory, total_memory, ip):
        ...
        if log != "":
            try:
                ...
                progress_pattern = getattr(
                    job, "progress_pattern", DefaultLocalProgressPattern
                )
                if progress_pattern != "":
...
try:
    opts, args = getopt.getopt(
        sys.argv[1:], "hp:vcs", ["help", "port=", "verbose", "init", "migrate", "reset"]
    )
    if len(args) != 0:
        usage()
        sys.exit(2)
    ...
    class WindowsService(win32serviceutil.ServiceFramework):
        ...
        def CheckForQuit(self):
            ...
            if not retval == win32event.WAIT_TIMEOUT:
                ...
                interfaces.IReactorCore.stop()

            interfaces.IReactorTime.callLater(1.0, self.CheckForQuit)
